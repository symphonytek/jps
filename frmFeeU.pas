...
uses mydataU, frmPreviewU, genperfU, myfuncU,dateutils,frxClass;
...
//if mydata.StudMast.Locate('StudId',mydata.tbfeeStudID.Value,[])then
  ...
  mydata.qryCustom.close;
  mydata.qryCustom.SQL.Text:='select ac_no,amount from adhocfee where studid='+Mydata.tbfeeStudID.asstring+' and paid=0';
  mydata.qryCustom.Open;
  setlength(mydata.adhocfee_arr,0);ind:=0;
  while not mydata.qryCustom.Eof do
  begin
      mydata.tbFeeDetail.insert;
      mydata.tbFeeDetailAc_NO.value:=mydata.qryCustom.Fields[0].value;
      mydata.tbFeeDetailAc_amt.value:=mydata.qryCustom.Fields[1].value;
      mydata.tbFeeDetail.post;
      setlength(mydata.adhocfee_arr,ind+1);
      mydata.adhocfee_arr[ind].ac_no:=mydata.qryCustom.Fields[0].value;
      mydata.adhocfee_arr[ind].amt:=mydata.qryCustom.Fields[1].value;
      ind:=ind+1;
    mydata.qryCustom.Next;
  end;
...
procedure TfrmFee.btnPrintClick(Sender: TObject);
var
tmpstr,info:string;
begin
mydata.frxReport1.LoadFromFile('frxFeeRec.fr3');
tmpstr:=sstr[mydata.tbfeeTERM.value];
        if mydata.tbfeeNOTERM.value>1 then
        tmpstr:=sstr[mydata.tbfeeTERM.asinteger-mydata.tbfeeNOTERM.asinteger+1]+'-'+tmpstr;
        info:=format('Payment for the month of %s',[tmpstr]);
(mydata.frxReport1.findobject('mmInfo') as TfrxMemoview).Text:=info;
...
frmpreview.Memo1.Lines.Clear;
frmpreview.Memo1.Lines.add(
        format('%7s %12s %13s %10s %10s %25s %20s %4s',
        ['Month','RecNo','Date','Amount','Chq No','Bank','Branch','Canc']));
frmpreview.Memo1.Lines.add(repchar('-',52));
with mydata do
begin
...
while not qrstudhist.Eof do
        ...
        frmpreview.Memo1.Lines.add(
        format('%7s %12s %13s %10.2f %10s %25s %20s %4s',
        [tmpstr,qrstudhistMR_NO.AsString,
        qrstudhistMR_DATE.asstring,
        qrstudhistTamt.value,mydata.qrStudHistCHQ_NO.Value,
        mydata.qrStudHistbank.Value,
        mydata.qrStudHistbranch.Value,mydata.qrStudHistcanc.value ]));
        qrstudhist.Next;
        end;
qrStudHist.close
end;
