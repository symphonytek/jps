...
pgSz:=trunc(strtoint(edPgSz.Text)*12/strtoint(rdgp2.Value));
...
exmdata.Query4.Filter:=exmdata.QrByBus.Filter;end;
//showmessage(exmdata.QrByBus.Filter+#13+exmdata.Query4.Filter);
...
//tmpStr2:='';
//for lnCnt:=1 to pgSz do tmpStr2:=tmpStr2+#13;
exmdata.Query4.Open;
while not exmdata.Query4.Eof do     //{for i:=1 to smRc do
begin
cnt:=11; i:=exmdata.Query4StudId.AsInteger; Redit.Lines.add('');
Redit.Lines.Add(#14+padc('REPORT CARD FOR '''+yrStr+#13+'~~~~~~~~~~~~~~~~~~~~',40));
Redit.Lines.add('');
Redit.Lines.add(format(' Name  : %-40s   Adm. No. : %10s',
                       [studAr[0,i],studAr[1,i]])); Redit.Lines.add('');
Redit.Lines.add(format(' Class : %-15s   Sec : %-13s      Roll No. : %3s',
                       [studAr[2,i],studAr[3,i],studAr[4,i]]));
Redit.Lines.add('');Redit.Lines.add(tmpStr1);Redit.Lines.add('');
...
Redit.Lines.add(tmpStr);inc(cnt);
end; //...if tmpStr<>
end; //...for k:=
//setlength(tmpStr2,pgSz-cnt);
tmpStr2:='';
for lnCnt:=1 to pgSz-cnt do tmpStr2:=tmpStr2+#13;//Redit.Lines.add('');
Redit.Lines.add(tmpStr2);
Redit.Lines.add('     Class Teacher                                     Principal');
exmdata.Query4.Next;
...
try
  ...
  Redt.Lines.Add(Redit.Text);
  {ShowMessage(inttostr(Redit.Lines.Count)+
      ' '+inttostr(Redt.Lines.Count));}
